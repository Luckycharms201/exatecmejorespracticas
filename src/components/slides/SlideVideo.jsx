...
/**
 * Slide de video (placeholder). Marco 16:9 a sustituir por el video real.
 * Entrada: kicker + marco con scale/blur-in; el botón de play late en
 * loop (pingpong) para señalar que ahí va un reproductor.
 */
export default function SlideVideo({ slide }) {
  const ph = slide.placeholders?.[0];

  const scope = useSlideTimeline((tl) => {
    tl.from(".video-kicker", { opacity: 0, y: 14, duration: 0.6 })
      .from(
        ".video-frame",
